...
# functions
credit <-
    read.csv("/Users/RyanQu/Documents/Workspace/R/train_data_newn.csv",   na.strings="?",
        col.names=sapply(0:9, function(x){return (paste("A",x,sep=""))}))
...
cat("\nRunning the validation test to determine the best k for the kNN
   method:\n")
n<-10 #Number of times a random sample is taken for each possible k 
...
# them:
lowKey=2000; highKey=2001; stepKey=1000
for (k in seq(lowKey,highKey,by=stepKey)){ # testing for each k
    ...
    for (i in 1:n){ # take n samples as training set
        ...
        knnModel <- kknn(A7 ~ A0 + A1 + A3 + A4 + A5 + A6 + A9, k=k,train=creditTrain,
        ...
        knnConfusion<-table(creditTest$A7, knnPred)
